...
public class Homework1 {
    ...
    public void streamPipeline5() {
        Arrays.stream(String.class.getDeclaredMethods())
                .filter(method -> Arrays.stream(method.getParameterTypes()).anyMatch(param -> boolean.class.equals(param)))
                .map(Method::getName)
                .sorted()
                .forEach(System.out::println);
    }
    ...
    public void streamPipeline6() {
        Arrays.stream(String.class.getDeclaredMethods())
                .filter(method -> Arrays.stream(method.getParameterTypes()).allMatch(param -> int.class.equals(param)))
                .map(Method::getName)
                .sorted()
                .forEach(System.out::println);
    }
    ...
    public String streamPipeline8() {
        return Arrays.stream(String.class.getDeclaredMethods())
                ...
                .max(Comparator.comparingInt(method -> method.getName().length()))
                .map(Method::getName)
                .get();
    }
